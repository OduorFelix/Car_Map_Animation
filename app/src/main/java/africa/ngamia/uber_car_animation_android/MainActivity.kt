package africa.ngamia.uber_car_animation_android

import africa.ngamia.uber_car_animation_android.util.AnimationUtils
import africa.ngamia.uber_car_animation_android.util.MapUtils
...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap?) {
        if (googleMap != null) {
            ...
            defaultLocation = LatLng(-1.2911279000, 36.7763820000)
    ...
    // Show Default Location of the User
    private fun showDefaultLocationOnMap(latLng: LatLng) {
        moveCamera(latLng)
        animateCamera(latLng)
    }

    // Used to Reposition the Camera to some LatLong (Pick Up Place)
    private fun moveCamera(latLng: LatLng) {
        googleMap.moveCamera(CameraUpdateFactory.newLatLng(latLng))
    }

    // Used to animate the movement of the camera from the current position to some
    // new position (current Driver or Individual SP Location)
    ...
    // Driver (Active SR) and SP App (Active SR) and Customer (Active SR)
